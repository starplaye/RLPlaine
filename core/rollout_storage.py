...
class RolloutStorageManager:
    ...
    def __init__(
        ...
        self.mu = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
        self.sigma = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
        self.durations = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def add_transitions(self, transition: TransitionManager):
        # check if the transition is valid
        if self.step >= self.num_transitions_per_env:
            raise OverflowError("Rollout buffer overflow! You should call clear() before adding new transitions.")
        ...
        self.rewards[self.step].copy_(transition.rewards.view(-1, 1))
        self.dones[self.step].copy_(transition.dones.view(-1, 1))
        self.durations[self.step].copy_(transition.durations.view(-1, 1))
    ...
    def compute_returns(self, last_values, gamma, lam, normalize_advantage: bool = True):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            tau = self.durations[step] 
            gamma_smdp = torch.pow(gamma, tau)

            # TD error: R_accumulée + gamma^tau * V(s_next) - V(s_curr)
            delta = self.rewards[step] + next_is_not_terminal * gamma_smdp * next_values - self.values[step]

            # GAE: On doit aussi utiliser gamma^tau ici pour l'avantage récursif
            gamma_lam_smdp = torch.pow(gamma * lam, tau)
            advantage = delta + next_is_not_terminal * gamma_lam_smdp * advantage
