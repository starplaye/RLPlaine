...
from core.rollout_storage import RolloutStorage, RolloutStorageManager
...
class PPOManager:
    ...
    def __init__(
        ...
        self.optimizer = optim.Adam(self.actor_critic.parameters(), lr=learning_rate)
        # Create rollout storage
        self.storage: RolloutStorageManager = None  # type: ignore
        self.transition = RolloutStorageManager.TransitionManager()
    ...
    def init_storage(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, action_shape):
        ...
        if self.rnd:
            ...
            rnd_state_shape = None
        # create rollout storage
        self.storage = RolloutStorageManager(
            num_envs,
            num_transitions_per_env,
            actor_obs_shape,
            critic_obs_shape,
    ...
    def process_env_step(self, rewards, dones, infos, durations):
        # Record the rewards and dones
        # Note: we clone here because later on we bootstrap the rewards based on timeouts
        self.transition.rewards = rewards.clone()
        self.transition.dones = dones.clone()
        self.transition.durations = durations.clone()
        ...
        # Bootstrapping on time outs
        if "time_outs" in infos:
            gamma_smdp = torch.pow(self.gamma, durations)
            
            self.transition.rewards += gamma_smdp * torch.squeeze(
                self.transition.values * infos["time_outs"].unsqueeze(1).to(self.device), 1
            )
